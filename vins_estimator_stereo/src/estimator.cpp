...
void Estimator::setParameter()
{
    ...
    //ProjectionFactor::sqrt_info = 100  * Matrix4d::Identity();
}
			     ...
			     const std_msgs::Header &header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image,image_r))
...
void Estimator::optimization()
{
    ...
    if(LOOP_CLOSURE)
    {
        ...
        {    
            for(int i = 0; i < WINDOW_SIZE; i++)
            {
				...
				Vector3d pts_i_r = it_per_id.feature_per_frame[0].point_right;
				Vector3d pts_j_r = Vector3d(retrive_data_vector[k].measurements[retrive_feature_index].x, retrive_data_vector[k].measurements[retrive_feature_index].y, 1.0);
				ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j, pts_i_r, pts_j_r);
    ...
    cout << "stereo termination_type:" << summary.termination_type << endl;
