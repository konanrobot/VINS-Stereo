	...
	double dx = 0, dy = 0, dz = 0, rx = 0, ry = 0, rz = 0;
	  ...
	  for (auto &imu_msg : measurement.first) {
	    ...
	    if (t <= img_t) {
	      ...
	      sensor_msgs::Imu imu;
	      imu.linear_acceleration.x = dx;
	      imu.linear_acceleration.y = dy;
	      imu.linear_acceleration.z = dz;
	  ...
	  ROS_DEBUG("processing vision data with stamp %f", img_msg->header.stamp.toSec());
	    ...
	    map<int, vector<pair<int, Vector3d>>> image_r;
		...
		double x1 = img_msg->points[i*2+1].x;
                double y1 = img_msg->points[i*2+1].y;
                double z1 = img_msg->points[i*2+1].z;
		
                ROS_ASSERT(z == 1);
                image[feature_id].emplace_back(camera_id, Vector3d(x,y,z));
		image_r[feature_id].emplace_back(camera_id, Vector3d(x1,y1,z1));
            }
            estimator.processImage(image, image_r, img_msg->header);
            /**
